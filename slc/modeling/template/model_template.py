...
def {{cmd_name}}(train, target_colname, scoring, n_splits, kfold_type, {{def_func_args}}):
    '''
    n_split>1のときはcvを使ってスコアをcsvで出力する
    戻り値はすべての訓練データを使って学習したモデル
    '''
    # モデルの生成
    clf = {{model_name}}({{model_args}})
...
def main(args):
    ...
    # 固有部分
{% for def_model_arg in def_model_args %}
    {{def_model_arg}}
{% endfor %}
...
# inputがstdinかファイル指定かで場合分け
    ...
    # 出力
    model = {{cmd_name}}(train_data, target_colname,
                         scoring, n_splits, kfold_type,{{model_args}})
