...
class SparsePCA():
	...
	def fit_transform(self,data):
		...
		transformed=pd.DataFrame(transformed,columns=["spca_"+str(i+1) for i in range(self.n_components)])
...
class Spca(Preprocess):
	...
	def spca(self,data):
		self.model=SparsePCA(cols=self.columns,n_components=self.n_components)
